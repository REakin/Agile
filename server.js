...
app.get('/clicker',(request, response)=>{
    ...
    if (request.signedCookies.ID == undefined){
        ...
        db.collection('Scores').findOne({_id: request.signedCookies.ID})
            .then(function (doc) {
                response.render('clicker.hbs',{
...
app.get('/getstats',(req,res)=>{
    db = mydb.getDb();
    db.collection('Scores').find({_id: req.signedCookies.ID}).toArray((err,result)=>{
...
app.post('/login',(request,response)=>{
    db = mydb.getDb();
    db.collection('Users').findOne({Username: request.body.Username, Password: request.body.Password})
        .then(function (doc) {
            if(doc == null){
...
app.post('/register',(request,response)=>{
  //creates a user and adds to our mongo database
   db = mydb.getDb();
    ...
    db.collection('Users').findOne({Username: request.body.Username}) //Check if user already exists.then(function (doc) {
            if(doc == null){
                //'User does not exist'
                ...
                db.collection("Scores").insertOne({ //Creates a game state for that user
                    _id: uid,
...
app.get('/getscores',(req,res)=>{
    db = mydb.getDb();
        db.collection('Scores').find().project({Username: 1, totalClicks: 1,_id:0}).toArray((err,result)=>{
            if (err) res.send('Error querying database!');
...
app.post('/update',(req,res)=> {
    try {
        ...
        db = mydb.getDb()
        db.collection('Scores').updateOne(query, info, function (err, res) {
            if (err) throw err;
        });
    }
    catch (e){
