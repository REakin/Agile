...
class Enemy {
    ...
    die(player) {
        ...
        player.gold += 1+this.container.state.rdnum;
...
class AutoWarrior {
    constructor(name, lvl, interval,player) {
        this.player = player;
        this.name = name;
    ...
    attack(enemy) {
        if (this.player.hp > 0) {
            enemy.takedamage(this.lvl,this.player);
        }
    }
    action(enemy) {
        this.actiontime = setInterval(this.attack.bind(this,enemy), this.interval)
    }
    teardown(){
